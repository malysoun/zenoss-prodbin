        ...
        if oldUids:
            with getSqlCursor() as cursor:
                ...
                cursor.executemany(sql,
                                   itertools.chain(
                                       ((myId, self.id, uid) for uid in oldUids),
                                       ((uid, self.remoteName(), myId) for uid in oldUids)
                                   ))
        self.setCount()
...
InitializeClass(RDBMSToManyRelationship)
